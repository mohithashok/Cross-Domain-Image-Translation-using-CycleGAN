...
def define_discriminator(image_shape: tuple):
    """
    Discriminator Model for the PatchGAN
    :param image_shape: The shape of the image passed to the discriminator
...
def resnet_block(n_filters: int, input_layer: np.array):
    """
    Utility function to generate ResNet blocks
    :param n_filters: Number of filters used
    :param input_layer: The input to the current block
    :return: The output from the current ResNet block
    """
    # weight initialization
    init = RandomNormal(stddev=0.02)
...
def define_generator(image_shape=(256, 256, 3), n_resnet=9):
    ...
    :param image_shape: The shape of the image generated by the generator
    :param n_resnet: Number of ResNet blocks used
...
def define_composite_model(g_model_1, d_model, g_model_2, image_shape):
    """
    Composite model for updating the generators by using adversarial and cycle loss
    :param g_model_1: Generator model that generates takes an image from the first domain an converts it to the second
    :param d_model: Discriminator model that checks if the image is from the second domain
    :param g_model_2: Generator model that generates takes the generated image and converts it back to the original
    :param image_shape: Shape of the image generated
...
def test(g_model_AtoB, g_model_BtoA, c_model_AtoB, c_model_BtoA, trainA, trainB):
    """
    Generate Day to Night Images
    """
    n_patch = 64
    for i in range(2):
        X_realA, _ = generate_real_samples(trainA, 1, n_patch)
        X_fakeB, _ = generate_fake_samples(g_model_AtoB, X_realA, n_patch)
        X_realB, _ = generate_real_samples(trainB, 1, n_patch)
